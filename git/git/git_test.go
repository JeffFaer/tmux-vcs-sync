...
func TestRepoAPI(t *testing.T) {
	newGitRepo := func(name string) (api.Repository, error) {
		dir := t.TempDir()
		if err := git.Command("-C", dir, "-c", "user.name=test", "-c", "user.email=test@example.com", "init", name).Run(); err != nil {
			return nil, fmt.Errorf("git init: %w", err)
		}
		dir = filepath.Join(dir, name)
		if err := git.Command("-C", dir, "-c", "user.name=test", "-c", "user.email=test@example.com", "commit", "--allow-empty", "--message", "Initial commit.").Run(); err != nil {
